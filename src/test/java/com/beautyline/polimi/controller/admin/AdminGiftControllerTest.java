...
public class AdminGiftControllerTest {
	...
	@MockBean
	private AdminGiftController giftController;
	...
	public void getGiftDone() {
		...
		// act
		ResponseEntity<GiftDTO> response = restTemplate.exchange("/admin/serviceOwned/111", HttpMethod.GET, request, GiftDTO.class);
	...
	public void getGiftsDone() {
		...
		// act
		ResponseEntity<Page<GiftDTO>> response = restTemplate.exchange("/admin/serviceOwned/list", HttpMethod.GET, request, type);
	...
	public void createDone() throws JsonProcessingException {
		...
		String requestBody = mapperJson.writeValueAsString(giftDTO);
		HttpEntity<String> request = new HttpEntity<>(requestBody, headers);

		// act
		ResponseEntity<GiftDTO> response = restTemplate.exchange("/admin/serviceOwned/create", HttpMethod.POST, request, GiftDTO.class);

		// assert
		Assertions.assertEquals(200, response.getStatusCodeValue());
		Mockito.verify(giftController, Mockito.times(1)).create(Mockito.any());
	}
	...
	public void updateDone() throws JsonProcessingException {
		...
		// act
		ResponseEntity<GiftDTO> response = restTemplate.exchange("/admin/serviceOwned/update", HttpMethod.POST, request, GiftDTO.class);

		// assert
		Assertions.assertEquals(200, response.getStatusCodeValue());
		Mockito.verify(giftController, Mockito.times(1)).update(Mockito.any());
	}
	...
	public void deleteDone() throws JsonProcessingException {
		...
		// act
		ResponseEntity<Void> response = restTemplate.exchange("/admin/serviceOwned/111", HttpMethod.DELETE, request, Void.class);

		// assert
		Assertions.assertEquals(200, response.getStatusCodeValue());
		Mockito.verify(giftController, Mockito.times(1)).delete(Mockito.any());
	}
